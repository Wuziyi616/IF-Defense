...
class Generator3D(object):
    ...
    def generate_mesh_sliding(self, data, return_stats=True):
        ...
        occ_values_x = np.array([]).reshape(
            0, r*n_crop_axis[1], r*n_crop_axis[2])
        for i in trange(n_crop):
            ...
            if self.upsampling_steps == 0:
                ...
                values = self.eval_points(
                    pp, c, vol_bound=vol_bound, **kwargs).detach().cpu().numpy()
            ...
            if (i+1) % n_crop_axis[2] == 0:
                occ_values_y = np.concatenate(
                    (occ_values_y, occ_values), axis=1)
                occ_values = np.array([]).reshape(r, r, 0)
            # along x axis
            if (i+1) % (n_crop_axis[2]*n_crop_axis[1]) == 0:
                occ_values_x = np.concatenate(
                    (occ_values_x, occ_values_y), axis=0)
                occ_values_y = np.array([]).reshape(r, 0, r*n_crop_axis[2])
    ...
    def encode_crop(self, inputs, device, vol_bound=None):
        ...
        for fea in self.vol_bound['fea_type']:
            ...
            if p_input.shape[0] == 0:  # no points in the current crop
                ...
                ind = coord2index(
                    p_input.clone(), vol_bound['input_vol'], reso=self.vol_bound['reso'], plane=fea)
                if fea == 'grid':
            ...
            input_cur = add_key(p_input.unsqueeze(0), index,
                                'points', 'index', device=device)
    ...
    def predict_crop_occ(self, pi, c, vol_bound=None, **kwargs):
        ...
        for key in self.vol_bound['fea_type']:
            ...
            p_n[key] = normalize_coord(
                pi.clone(), vol_bound['input_vol'], plane=key).unsqueeze(0).to(self.device)
        pi_in['p_n'] = p_n
